...
SELECT @CREATE_TEMP_TABLE_VALUES += 
	...
	(
		CASE WHEN CAST(CHARACTER_MAXIMUM_LENGTH AS VARCHAR(MAX)) IS NULL THEN DATA_TYPE 
        ELSE DATA_TYPE + '(' + 
		    CASE WHEN CAST(CHARACTER_MAXIMUM_LENGTH AS VARCHAR(MAX)) = '-1' THEN 'MAX' ELSE CAST(CHARACTER_MAXIMUM_LENGTH AS VARCHAR(MAX)) END + ')' END
    ) 
	+ ' NULL,'
FROM INFORMATION_SCHEMA.COLUMNS
WHERE 
	TABLE_NAME = @TABELA+'010' AND
...
DECLARE @CREATE_TEMP NVARCHAR(MAX) = 
	N'DROP TABLE IF EXISTS ##T_' + @TABELA + '_DW_SEL;' + 
	'CREATE TABLE ##T_' + @TABELA + '_DW_SEL' + 
	'
	(
		'+ @TABELA +'_EMPRESA VARCHAR(2) NULL,
...
BEGIN
   ...
   DECLARE @SELECT NVARCHAR(MAX) = N'
    ...
    WHERE ' + QUOTENAME(@COL_DT) + ' >= GETDATE() - ' + @VL_CORTE + ';
   '
